...
func outputDeaconPatrolContext(ctx RoleContext) {
	...
	cfg := PatrolConfig{
		...
		WorkLoopSteps: []string{
			...
			"At cycle end:\n   - If context LOW:\n     * Report and loop: `" + cli.Name() + " patrol report --summary \"<brief summary of observations>\" --steps \"<all step_id:OK|SKIP pairs>\"`\n     * The --steps audit is required; this closes the current patrol and starts a new cycle\n   - If context HIGH:\n     * Send handoff: `" + cli.Name() + " handoff -s \"Deacon patrol\" -m \"<observations>\"`\n     * Exit cleanly (daemon respawns fresh session)",
		},
	}
